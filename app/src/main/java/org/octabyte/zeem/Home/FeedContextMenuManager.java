...
public class FeedContextMenuManager extends RecyclerView.OnScrollListener implements View.OnAttachStateChangeListener {
    ...
    public void toggleContextMenuFromView(View openingView, String postSafeKey, String feedSafeKey, Long userId, Long postId, Boolean isPublic, Boolean tagged, Boolean tagApproved, FeedContextMenu.OnFeedContextMenuItemClickListener listener) {
        if (contextMenuView == null) {
            showContextMenuFromView(openingView, postSafeKey, feedSafeKey, userId, postId, isPublic, tagged, tagApproved, listener);
        } else {
            hideContextMenu();
        }
    }

    private void showContextMenuFromView(final View openingView, String postSafeKey, String feedSafeKey, Long userId, Long postId, Boolean isPublic, Boolean tagged, Boolean tagApproved, FeedContextMenu.OnFeedContextMenuItemClickListener listener) {
        if (!isContextMenuShowing) {
            isContextMenuShowing = true;
            contextMenuView = new FeedContextMenu(openingView.getContext(), tagged, tagApproved, userId);
            contextMenuView.bindToItem(postSafeKey, feedSafeKey, postId, isPublic);
    ...
    private void setupContextMenuInitialPosition(View openingView) {
        ...
        int additionalBottomMargin = 0;//Utils.dpToPx(16);
